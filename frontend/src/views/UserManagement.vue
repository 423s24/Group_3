<template>
  ...
  <Modal v-if="isModalVisible" :modalTitle="'New User'" @close="hideModal">
    <div class="border-t border-gray-200 rounded-b">
      <form id="new-user-form" @submit.prevent="createNewUserFormSubmit">
        <div class="grid md:grid-cols-2 gap-2">
          <div>
            <p>First Name</p>
            <input v-model="newUserFirstName" type="text" id="firstName"
              class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
          </div>
          <div>
            <p>Last Name</p>
            <input v-model="newUserLastName" type="text" id="lastName"
              class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
          </div>
          ...
          <div class="md:col-span-3 md:grid md:grid-cols-3 gap-2">
            <div>
              <p>Username</p>
              <input v-model="newUserUsername" type="text" id="username"
                class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
            </div>
            <div>
              <p>Password</p>
              <input v-model="newUserPassword" type="password" id="password"
                class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
            </div>
            <div>
              <p>Role</p>
              <select v-model="newUserIsAdmin" name="isAdmin" id="isAdmin" class="w-full">
      ...
      <div class="flex flex-row justify-center">
        <div class="w-2/3 p-4 m-4">
          <div class="bg-white p-4 m-3 w-full">
            ...
            <table class="w-full border-2 border-bg-blue-900">
              ...
              <tbody class="rounded">
                <tr v-for="user in allUsers" :key="user._id" class="border-b-2 border-bg-blue-900 h-14"
        ...
        <div class="w-1/3 p-4 flex flex-col justify-start">
          ...
          <div class="bg-white rounded-lg p-4 border-2 border-bg-blue-900 w-full">
            ...
            <form @submit.prevent="updateUserFormSubmit">
              <div class="flex items-center mb-2">
                <label for="selectedUserFirstName" class="w-1/3 text-right mr-3">First Name</label>
                <input v-model="selectedUser.firstName" type="text" id="selectedUserFirstName"
              ...
              <div class="flex items-center mb-2">
                ...
                <input v-model="selectedUser.lastName" type="text" id="selectedUserLastName"
                  class="border-2 border-gray-300 rounded w-2/3 p-2" :disabled="!isEditingSelectedUser">
              </div>
              <div class="flex items-center mb-2">
                <label for="selectedUserUsername" class="w-1/3 text-right mr-3">Username</label>
                <input v-model="selectedUser.username" type="text" id="selectedUserUsername" value="JDoe"
                  class="border-2 border-gray-300 rounded w-2/3 p-2" :disabled="!isEditingSelectedUser">
              </div>
              <div class="flex items-center mb-2">
                <label for="selectedUserRole" class="w-1/3 text-right mr-3">Role</label>
                <select v-model="selectedUser.isAdmin" name="userRole" id="selectedUserRole"
                  class="border-2 border-gray-300 rounded w-2/3 p-2" :disabled="!isEditingSelectedUser">
...
export default defineComponent({
  ...
  methods: {
    ...
    createNewUserFormSubmit() {
      store.dispatch('userModule/createNewUser', {
    ...
    updateUserFormSubmit() {
      store.dispatch('userModule/updateUser', this.selectedUser);
      this.isEditingSelectedUser = false;
  ...
  async created() {
    try {
      await store.dispatch('userModule/getAllUsers');
      this.allUsers = store.state.userModule.allUsers;
    } catch (error) {
      console.error(error);
    }
  }
